...
function averageKalshiPrices(a: string | number, b: string | number, responseUnits?: string): number {
    const average = averageDecimals(String(a), String(b));
    const normalizedUnits = responseUnits?.toLowerCase();
    return normalizedUnits === 'cent' || normalizedUnits === 'cents' ? average / 100 : average;
}
...
export class KalshiNormalizer implements IExchangeNormalizer<KalshiRawEvent, KalshiRawEvent> {
    ...
    normalizeRawMarket(event: KalshiRawEvent, market: KalshiRawMarket): UnifiedMarket | null {
        ...
        if (market.last_price_dollars != null) {
            ...
            price = averageKalshiPrices(market.yes_ask_dollars, market.yes_bid_dollars, market.response_price_units);
        } else if (market.yes_ask_dollars != null) {
            price = parseKalshiPrice(market.yes_ask_dollars, market.response_price_units) ?? 0;
