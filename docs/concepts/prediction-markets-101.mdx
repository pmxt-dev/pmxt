...
You run `pmxt-core` on your own machine, supply raw venue credentials, and the SDK talks directly to the venues. No API key, no escrow, no PMXT in the trade path. See [Hosted vs Self-hosted](/concepts/hosted-vs-self-hosted) for the comparison.
