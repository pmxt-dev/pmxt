...
| **Who holds venue credentials?** | Non-custodial `PreFundedEscrow` contract holds USDC; PMXT holds no venue credentials | You, on your machine |
...
- You want the **non-custodial `PreFundedEscrow`** model instead of managing venue-native accounts.
...
- You **prefer raw venue credentials** — Polymarket L2 API keys, Kalshi RSA keys, Smarkets sessions — and don't want PMXT in the settlement path at all.
- You have **regulatory requirements** that mandate direct counterparty status with the venue.
...
This is useful if you trust PMXT enough for catalog and price data but want execution to settle directly against the venue.
